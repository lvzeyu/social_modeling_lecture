...
network
network
"random" "fully connected" "small-world"
0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot 100 * count turtles with [riot? = true] / count turtles"
...
315
555
These parameters are only relevant for the normal distribution
12
0.0
1
...
335
456
These parameters are only relevant for the uniform distribution
12
0.0
1
...
345
375
This parameter is only relevant for the small-world network
12
0.0
1
...
345
325
This parameter is only relevant for the small-world and random networks
12
0.0
1
...
This model reproduces [Mark Granovetter's (1978)](http://www.jstor.org/stable/2778111) threshold model of collective behavior with different network structures.
...
* The _threshold-distribution_ chooser allows to extract the agents' thresholds following different distributions. Current possibilities are the "uniform" and the "normal" distributions.
...
@#$#@#$#@
<experiments>
  <experiment name="mean-effect" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="200"/>
    <metric>final-proportion</metric>
    <metric>ticks</metric>
    <enumeratedValueSet variable="network">
      <value value="&quot;fully connected&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="threshold-distribution">
      <value value="&quot;normal&quot;"/>
    </enumeratedValueSet>
    ...
    <steppedValueSet variable="mean-threshold" first="1" step="1" last="60"/>
  </experiment>
  <experiment name="diversity-effect" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="200"/>
    <metric>final-proportion</metric>
    ...
    <enumeratedValueSet variable="mean-threshold">
      <value value="25"/>
    </enumeratedValueSet>
    <steppedValueSet variable="sd-threshold" first="1" step="1" last="30"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
