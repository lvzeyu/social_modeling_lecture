...
> ネットワークとは、ノードと呼ばれる相互接続された実体と、エッジと呼ばれる、それらを接続するものの集合を表現したもの
...
> ネットワークとは、ノードと呼ばれる相互接続された実体と、エッジと呼ばれる、それらを接続するものの集合を表現したもの
- ネットワークに関する異なる分野では、しばしば独自の専門用語を用いている
    - ネットワークをグラフ(graph)と呼ぶ
    - ノードをバーテックス(vertex)と呼ぶ
    - エッジをリンク(link)と呼ぶ

- ネットワークは、ノードの総数$N$とエッジの総数$L$によって特徴づけられる
  - $N$はシステムを構成する個々の要素の数を表すため、ネットワークのサイズと呼ぶ
</v-clicks>
...
- エッジには様々な種類があり、それによってネットワークが定義される
...
- [ネットワークの基本統計量の説明](https://colab.research.google.com/github/lvzeyu/social_modeling_lecture/blob/main/lecture8/Graph_measurements.ipynb)
</v-clicks>
