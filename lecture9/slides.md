...
title: 行動科学概論 - ネットワークモデル
info: |
  ## 行動科学概論
  社会科学におけるモデル入門: ネットワークモデル
...
> ネットワークの特徴がどのように現れるかについての直感や仮説を取り込んでモデルを構築し、そのモデルに従って構築されたネットワークと現実のネットワークを比較することで、何がモデルによって再現できて、何が再現できないかを確認する
...
- 比較的小規模な部分ネットワークのみで構成されるネットワークから、ほとんどすべてのノードを含む巨大ネットワークに成長し、変化は徐々に起こる
...
Erdos-Renyi (ER)モデルは、ノード数$N$とリンク生成確率$p$の二つのパラメータを持つモデルであり、$G(N, p)$ と表される
...
- a. ランダムネットワークにおける次数の確率分布は、平均次数$<k>$付近に高いピークをもち、その両側で急速に減衰するベル型の曲線になる
- 多くの現実ネットワークはかなり異なる分布になる
    - b.世界の航空網ネットワーク
  - c.現実ネットワークとランダムネットワークとの比較
</v-clicks>
...
- あるノードの隣接ノード同士がつながっている確率は、共通の隣接ノードがあるかどうかに依存せず、どのペアでも同じであるため$p$となる
    - 個々のノードの局部的なクラスターは多少ずれることはあるが、全てのノードの平均値は$p$でよく近似できる

- ランダムネットワークで現実のネットワークのような疎なネットワークを記述する場合、$p$は非常に小さい値になる
...
- 現実のネットワークはランダムネットワークとは異なる特徴を持つ
  - ランダムネットワークは平均経路長が短いが、クラスタ係数は実際のネットワークより大幅に小さい値になる
- クラスタリングと短距離は矛盾する性質?
    - クラスタ係数が高いネットワークでは、局所的に三角形が多く、密なつながりがある
    - ローカルな接続は密だが、他のクラスタとの接続が少なくなりがち →　ネットワーク全体では平均距離が伸びやすい？
- スモールワールドモデル（Watts–Strogatzモデル）
    - 短い経路長と大きなクラスタ係数の両方の特徴を同時に持つネットワークを生成する
</v-clicks>
...
- 6次の隔たり：世界のどんな2人も短い知人の連鎖で繋がっている
  - 社会ネットワークの直径は小さく、平均経路長はさらに短い
  - 「6」という数字は、Frigyes Karinthy という小説家が最初に用いた
...
- 実証的なアプローチ：1967年の心理学者スタンレー・ミルグラムの「スモールワールド実験」
  - 見知らぬ人同士が、どれくらいの“社会的距離（知人関係のつながり）”で結ばれているのかを測定しようとした
...
- 再配線確率は$p$、ネットワーク内のリンクの総数は$L$である場合、再配線されるリンク数の期待値は$pL$である
  ...
  - $p=1$の場合は、基本的に全てのリンクはランダムに再配線され、ランダムネットワークに等価である
- $p$は小さすぎず、大きすぎないとき
...
- [Demo](https://www.netlogoweb.org/launch#https://www.netlogoweb.org/assets/modelslib/Sample%20Models/Networks/Small%20Worlds.nlogox)
...
- 実験結果から$p$を変化させた際のネットワーク特性の推移が確認できる
    - **$p = 0$（規則的な格子）**: クラスタ係数$C$が高く、平均経路長$L$も長い
    - **$p$を僅かに増加（$0 < p \ll 1$）**: $C$はほとんど変化しないが、$L$が急激に短縮される
        - わずかな再配線でショートカットが生まれ、全体の距離が劇的に縮まる
    - **$p = 1$（ランダムネットワーク）**: $L$は最小だが、$C$も低くなる
- **スモールワールド領域**: $C$が高く$L$が短い中間的な$p$の範囲
    - 繰り返し実験により、この領域が再現性を持つことが確認できる

</v-clicks>

<p v-click style="color: #3E1586; font-size: 1.5em; text-align: center;">
  BehaviorSpaceにより、スモールワールド現象が特定の$p$範囲で安定して出現することが実証できる
</p>
...
**NetLogoのSmall Worldsモデルを用いたシミュレーション実験**
...
1. **モデルの準備**
   - NetLogo で `Small Worlds_with_experienment.nlogox` を開く
   - `num-nodes` スライダーを **100** に設定する

2. **BehaviorSpace の設定**
   - Tools → BehaviorSpace → New（またはEdit）
   - Setup commands に以下を入力：
     ```
     set num-nodes 100
     setup
     ```
   - Vary variables as follows に入力：
     ```
     ["rewiring-probability" [0 0.005 1]]
     ```
   - Repetitions: **5**、Go commands: `rewire-all`
   - Metrics: `clustering-coefficient`、`average-path-length`

3. **実験の実行と結果の保存**
   - Run → Table output と Stats output を選択してCSVを保存
...
**① BehaviorSpace 設定画面のスクリーンショット**
- 設定した変数・繰り返し数・メトリクスが確認できること

**② 結果の図（必須）**
- 横軸：再配線確率 $p$（対数スケール推奨）
- 縦軸：$C(p)/C(0)$（クラスタ係数）と $L(p)/L(0)$（平均経路長）を同一グラフに表示
- エラーバー（5回の繰り返しの標準偏差）を含めること

**③ 考察（200字以上）**
以下の点を中心に論述すること：
- $p$ の増加に伴う $C(p)$ と $L(p)$ の変化パターン
- クラスタ係数が高いまま平均経路長が急落する $p$ の範囲はどこか
- その結果がスモールワールド現象とどのように関連するか

</v-clicks>

<!--
採点基準（参考）
- 設定画面：10点（num-nodes=100・p範囲・5repetitions が確認できること）
- 図：50点（正規化・エラーバー・対数スケール・両指標の表示）
- 考察：40点（現象の記述・スモールワールド領域の特定・社会的含意）
-->