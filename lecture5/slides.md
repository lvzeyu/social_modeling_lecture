...
> 孟母三遷: 中国、戦国時代の思想家孟子の母が、孟子の教育のために三度も住居を遷(うつ)ったとの故事をいう。孟母は初め墓地の近くに居を定めたが、孟子が葬式のまねばかりするので、市場の近くに転居した。すると今度は商売人のまねをして遊ぶので、ここもわが子のためにふさわしい所ではないと、学校のそばに居を移すと、孟子が喜々として礼儀作法のまねをするようになったので、孟母はこここそがわが子のいるべき所だといって、ついにここに住居を定めたと伝える。
 </v-clicks>
...
- **過程**
    ...
    - シミュレーションでは、複数ラウンドが実行される
        - 各ラウンドにおいて、すべての住人は、満足度関数を計算し、それが$0$であった場合は、その時点で住人が配置されていない場所のリストからランダムに選ばれる場所に移動する
...
- **設計**
    ...
    - 観察指標は、 シミュレーション終了時における2次元格子空間に配置された人種の分布リストである
...
- $x_i$: セル $i$ におけるエージェントの「色（属性）」を表す値
  - $x_i = 1$ のとき、そのセルには青のエージェントであり、$x_i = 0$ のとき、そのセルには赤のエージェントである
