...
<!--
von Neumann近傍では、中心のセルからマンハッタン距離1にあるセルを近隣とみなします。具体的には、2次元格子（正方格子）上で、あるセルの上下左右(じょうげさゆう)の4つのセルを近傍とします
-->
...
<!--
従来のシェリングモデルでは、エージェントは格子状の空間（例えば、正方形グリッド）上に配置され、上下左右の近隣セルを考慮して移動を決定します。

しかし、現実の都市空間はより複雑であり、道路網や地形、建物の配置などが影響を及ぼします。このため、より現実的な空間表現を取り入れたモデルの拡張が行われています。

実際の地理情報システム（GIS）データを使用して、エージェントの居住地や移動可能な領域をより現実的にモデル化しています。これにより、都市の地理的特徴や土地利用パターンがエージェントの行動に影響を与えるようになります。

道路や河川などの地理的障壁を考慮し、エージェントの移動に制約を設けています。これにより、実際の都市における物理的な制約がモデルに反映されます。
-->
...
<!--
この研究では、エージェントが持つ属性や選好を多様化し、より現実的なシナリオをモデル化しています。
- 複数のグループ: 従来の2グループ（例：白人と黒人）から、より多様なグループ構成に拡張しています
- 社会的文脈の考慮: 実際の調査データを活用し、人々の居住選好や行動パターンをモデルに反映させています
    - 人種による同質性また近隣の選好も異なることはわかりました
- 選好の強さの違い: エージェントごとに異なる選好の強さを持たせ、同じグループの隣人を好む度合いを変化させています
-->
