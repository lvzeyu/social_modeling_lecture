...
人間は、多様で社会的、目的を持ち、学習し、自律的に変化する存在である。このような存在を理解・モデル化するためには、単純な合理的行為者モデルでは不十分である。

- まず、限定合理性という考え方は、人間が「完全情報を持ち、計算能力に制限がない理想的な意思決定者」ではなく、情報や計算能力に制限を持つ中で、満足可能な選択を行う存在であることを認めた点で、従来の完全合理性モデルよりは現実に近づいた。しかしそれでも、なお現実の人間の行動を十分に捉えきれていない。

- その問題点を克服数ために、「不完全情報を持つ合理的行為者」として人間を拡張的にモデル化される手法も提案されています
    - 例えば、プレイヤーが他のプレイヤーのタイプ（好みや情報）を知らないが、ある程度の**確率分布（信念）**を持っている状況。

- しかし、たとえ「不完全情報を持つ合理的行為者」として人間を拡張的にモデル化しても、人間の目的は常に明確でも一貫的でもないからである。人間は、多様な価値観や動機を持ち、それらは状況や文脈に応じて変化しうる。そのため、一つの効用関数で全ての価値判断を一貫して順位づけることは困難である。

- 合理的選択理論が前提とする「独立性」や「推移性」（たとえばAよりB、BよりCを好むなら、AよりCを好むべき）といった理論的前提は、実際の人間の判断において頻繁に破られる。これは、人間の意思決定が文脈依存的であり、感情や社会的要因にも強く影響されることを示している。
...
- BよりC（自己安全＞家族）→ まず自分の安全を確保

- AよりC（職務＞自己安全）→ 避難誘導に向かう

このように、選好が循環し、A > B > C > Aという推移性が破られる状況が生じます。これは合理的選択理論が前提とする一貫した選好構造（推移性、独立性）に反するものです
-->
...
個々の構成要素の単純なルールに従った行動の積み重ねが、集団レベルで予期せぬ複雑なパターンや構造を生み出す現象
-->
