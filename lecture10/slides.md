...
- ネットワークの基本概念
    - ノードとリンクによる複雑な対象を比較的に単純な構造で表現する
    - 次数、最短経路長、クラスター係数などの指標でネットワークの特徴を理解する
- ネットワークモデル
    - ネットワークの特徴がどのように現れるかについての直感や仮説を取り込んでモデルを構築し、そのモデルに従って構築されたネットワークと現実のネットワークを比較することで、何がモデルによって再現できて、何が再現出来ないかを確認する
        - ネットワークモデルによって現実世界のネットワークがどのようなメカニズムで成り立っているのかについての知見を得ることができる
    - WSモデル: 適切なランダム性$p$を設定することで、短い最短経路長と大きいクラスター係数という二つの望ましい特徴を持つネットワークを生成することが可能である
...
- 各ノードに接続されたスタブの数はそのノードの次数に等しいので、各ノードは最終的に望ましい次数を持つことになる
    - 二つのノード間に複数のリンクがあるネットワーク$(c)$や自己ループがあるネットワーク$(d)$を生成する可能性もあるので、場合によっては制御する必要がある
...
- 一方、現実のネットワークはノードやリンクが時間とともに現れたり消えたりすることが多く、**動的**である
...
- 新しいノードやリンクが追加されることにつれて、ノードの次数は大きくなっていく
  - 新たに追加されたノードは、優先的選択に基づいて既存ノードにリンクを張る
  - 既に多くのリンクを持つノードがさらにリンクを獲得する確率が高く、次数は非対称に拡大していく
  - 結果として、ノードごとに大きな次数の格差が生まれ、一部のノードがハブ化していく
  - [Demo](https://www.netlogoweb.org/launch#https://www.netlogoweb.org/assets/modelslib/Sample%20Models/Networks/Preferential%20Attachment.nlogox)
</v-clicks>
...
- 拡散はごく少数のノードで起こることもあるし、ネットワークの大部分を含む大域的拡散が発生することもある
</v-clicks>
...
- 移動ネットワーク：ある居住地域から特定の移動先への訪問者数によって構築したネットワーク
    - 米国の10大都市圏における約9,800万人のモバイルデバイスの位置情報データを使用
- SEIRモデルの適用: ある居住地域から特定の移動先への移動を通じて、感染者が他の住民と接触し、感染が広がる
...
- 均質なネットワークであっても閾値効果を持たず、伝達確率が低くても噂は多くの人に届きわたる
- 非均質なネットワーク(例：スケールフリー型)においても閾値はなく、最終的に噂を知る人の数は、同じノード数とリンク数を持つ均質なネットワークより少なくなる。
    - 噂が早い段階でハブに到達し、多くの人と接触することによってすぐに抑制者になるためである
...
Simple contagion
  - 一対一の接触で感染が成立する可能性がある
  - 未感染のAgentは、感染済みのAgentから影響を受けると、確率$p$で感染する

Complex contagion
  - 感染では複数の接触が必要
      - ある行動や考えが広がるためには、複数の人から同じ情報や影響を受けることが必要な場合がある
      - 未感染のAgentは、少なくとも2人以上の隣人が感染している場合確率$p$で感染する
...
![contagion types](./image/contagion_type.png)
...
<img src="./image/contagion_results.png" width="800" />

- 弱い紐帯やLong tieは単純な伝播には有効だが、複雑な伝播ではむしろ拡散を妨げる場合がある
...
- ネットワークモデルは、複雑な現実世界を単純化されたルールで再現し、その本質的な特徴がなぜ生まれるのかを理解することに役に立つ
...
- ネットワーク応用：SIRモデルとの組み合わせを例として
    - より現実に近いネットワークを適用することで、感染拡大のメカニズムを説明し、最適な介入を設計する
