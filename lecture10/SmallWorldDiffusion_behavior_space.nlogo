...
to go
  ...
  [
    ...
    ask link-neighbors with [not infected?]
      [
        if ( random-float 1 <= prob-infection and ((not complex-contagion?) or (count link-neighbors with [infected? = true] > 1)))  ;; infect with probability p
        [
          set infected? true
          set color yellow
          set size infected-size
...
<experiments>
  <experiment name="time-to-spread-simple" repetitions="100" runMetricsEveryStep="false">
    ...
    <metric>ticks</metric>
    <metric>num-infected / count turtles</metric>
