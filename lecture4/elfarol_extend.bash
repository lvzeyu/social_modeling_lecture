globals [
  ...
  crowded-count     ;; 混雑状態になったtickの累計回数
]
...
turtles-own [
  ...
  tolerance       ;; 個体ごとの混雑許容閾値 (overcrowding-threshold にノイズを加えたもの)
]
...
to setup
  ...
  set history n-values (memory-size * 2) [random population]
  ...
  create-turtles population [
    ...
    ;; tolerance-sd スライダーに基づく個体差を付加する
    set tolerance overcrowding-threshold + random-normal 0 tolerance-sd
    update-strategies
  ]
...
to go
  ...
  ask turtles [
    ...
    set attend? (prediction <= tolerance)  ;; 個体ごとの tolerance で判断する
  ]
  ...
  if attendance > overcrowding-threshold [
    ...
    set crowded-count crowded-count + 1
  ]
